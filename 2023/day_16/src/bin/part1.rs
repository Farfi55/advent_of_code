...
fn part1(input: &str) -> usize {
    ...
    let energized = energize(&map);

    println!("energized:\n{}", energized_to_str(&energized));
...
fn energize(map: &[Vec<char>]) -> Vec<Vec<bool>> {
    let mut energized_in_direction: Vec<Vec<Vec<bool>>> =
        vec![vec![vec![false; 4]; map[0].len()]; map.len()];

    let start = (0, 0);
    energized_in_direction[start.0][start.1][dir_to_index(RIGHT)] = true;

    energize_rec(map, start, RIGHT, &mut energized_in_direction);
...
.#...#.#.."#;
        ...
        let energized: Vec<Vec<bool>> = energize(&map);
