...
fn extrapolate_next(readings: Vec<isize>) -> isize {
    ...
    let diff_vec = &diff_vec[diff_vec.len() - iterations..];
    let mut running_estimate = 0;
    for i in 0..iterations {
        running_estimate = running_estimate + diff_vec[i];
    }
    running_estimate
}
...
fn build_diff_vec(mut readings: Vec<isize>) -> (Vec<isize>, usize) {
    // for every iteration,
    // take i up to readings.len - iteration
    // readings[i] = readings[i] - readings[i+1]
    // if all_zero(readings) { break }

    let len = readings.len();
    let mut iteration = 1;
    loop {
        for i in 0..len - iteration {
            readings[i] = readings[i + 1] - readings[i];
        }

        if all_zero(&readings[..len - iteration]) {
            break;
        }
...
"#;
    ...
    fn build_diff_vec_correctly() {
        let test_cases = vec![
            (vec![0, 3, 6, 9, 12, 15], (vec![0, 0, 0, 0, 3, 15], 2)),
            (vec![1, 3, 6, 10, 15, 21], (vec![0, 0, 0, 1, 6, 21], 3)),
            (vec![10, 13, 16, 21, 30, 45], (vec![0, 0, 2, 6, 15, 45], 4)),
        ];
