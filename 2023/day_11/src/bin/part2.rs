...
impl Observation {
    ...
    fn distance_between(
        ...
        let row_distance = row_max - row_min
            + (self.rows_empty_count[row_max] - self.rows_empty_count[row_min]) * empty_space;
        let col_distance = col_max - col_min
            + (self.cols_empty_count[col_max] - self.cols_empty_count[col_min]) * empty_space;
        row_distance + col_distance
    }

    fn sum_distance_between_all_galaxies(&self, empty_space: usize) -> usize {
        let mut sum: usize = 0;
        for i in 0..self.galaxies.len() - 1 {
            for j in i + 1..self.galaxies.len() {
                sum += self.distance_between(self.galaxies[i], self.galaxies[j], empty_space);
            }
        }
        sum
    }
...
fn part2(_input: &str) -> usize {
    ...
    observation.sum_distance_between_all_galaxies(1_000_000 - 1)
}
...
"#;
    ...
    #[test]
    fn test_distance() {
        let obs = Observation::new(EXAMPLE_INPUT);
        assert_eq!(obs.sum_distance_between_all_galaxies(10 - 1), 1030);
        assert_eq!(obs.sum_distance_between_all_galaxies(100 - 1), 8410);
    }
