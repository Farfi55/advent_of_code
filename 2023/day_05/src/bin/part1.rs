...
fn parse_almanac(input: &str) -> AlmanacMaps {
    ...
    almanac.seeds = seeds_raw.map(|seed| seed.parse().expect("should be a number")).collect();
...
fn part1(input: &str) -> usize {
    ...
    almanac.seeds.iter().map(|seed| almanac.seed_to_location(*seed)).inspect(|location| println!("location: {}", location))
